...
char *format_extension(const char *extension) {
  ...
  /* The first thing to do is to create the name of the file where the template
   * string should be stored. Because this is only usefull at the beginning, we
   * will reuse the variable long later on to hold the size of the format file.
   * This is why it is a long and not an int. */
  long length = strlen(extension) + strlen(home_path);
  /* We allocate on the stack an array big enough to fit the name of the file we
  ...
  /* We have enough room in out buffer, so we simply copy the format string from
   * our file. We check for errors at the same time. Because we have a single
   * condition here, the call to fread will always take place. */
