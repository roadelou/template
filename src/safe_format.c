...
int safe_format(FILE *stream, const char *format, const char *author,
    ...
    for (cursor = 0; cursor < length; cursor++) {
        if (*(format + cursor) == '%') {
            /* We take a decision based on the next character. Note that if '%'
             * is the last character of the string, the next character that we
             * will pick is the termination byte, so there will be no
             * segmentation fault here. */
            switch (*(format + cursor + 1)) {
            case '1':
                /* The format for the author. */
                if (fputs(author, stream) == EOF) {
                    /* A problem appeared, we cannot write to the file and have
                     * to exit.
                     */
                    return ERROR;
                }
            ...
            case '%':
                ...
                break;
            default:
                /* There is no interpretation for this '%', we just leave it as
                 * is. */
                if (fputc('%', stream) == EOF) {
                    /* A problem appeared, we cannot write to the file and have
                     * to exit.
                ...
                /* We don't move the cursor because we haven't used the next
                 * character yet. */
                break;
            }
            ...
            if (fputc(*(format + cursor), stream) == EOF) {
                /* A problem appeared, we cannot write to the file and have to
                 * exit. */
                return ERROR;
            }
        }
