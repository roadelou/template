...
int safe_format(FILE *stream, const char *format, const char *author,
  ...
  for (cursor = 0; cursor < length; cursor++) {
    if (*(format + cursor) == '%') {
      /* We take a decision based on the next character. Note that if '%' is the
       * last character of the string, the next character that we will pick is
       * the termination byte, so there will be no segmentation fault here. */
      ...
      default:
        ...
        /* We don't move the cursor because we haven't used the next character
         * yet. */
        break;
      }
